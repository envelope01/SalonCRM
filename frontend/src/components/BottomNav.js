  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 flex justify-center">
      <div className="relative w-full max-w-3xl">
        <div className="absolute inset-0 border-t border-gray-200/50 bg-white/80 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] backdrop-blur-xl" />

        <div className="relative flex items-center justify-around px-2 pt-2 pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
          {navItems.map((item) => (
            <NavLink key={item.to} to={item.to} className="flex-1">
              {({ isActive }) => (
                ...
                >
                  ...
                  <span className={`mt-1 text-[10px] font-bold transition-colors duration-300 ${
